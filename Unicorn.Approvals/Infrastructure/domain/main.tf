...
locals {
  event_bus_name      = "unicorn-approvals-eventbus-${var.stage}"
  registry_name       = "${data.aws_ssm_parameter.unicorn_approvals_namespace.value}-${var.stage}"
...
resource "aws_cloudwatch_event_bus_policy" "approvals_event_bus_publish_policy" {
  event_bus_name = aws_cloudwatch_event_bus.approvals_event_bus.name
  statement_id   = "OnlyApprovalsServiceCanPublishToEventBus-${var.stage}"
  ...
  statement = jsonencode({
    ...
    Condition = {
      StringEquals = {
        "events:source" = [data.aws_ssm_parameter.unicorn_approvals_namespace.value]
      }
    }
  })
}
...
resource "aws_cloudwatch_event_bus_policy" "rule_management_policy" {
  event_bus_name = aws_cloudwatch_event_bus.approvals_event_bus.name
  statement_id   = "OnlyRulesForApprovalsServiceEvents-${var.stage}"
  ...
  statement = jsonencode({
    ...
    Resource = "arn:${data.aws_partition.current.partition}:events:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:rule/${aws_cloudwatch_event_bus.approvals_event_bus.name}/*"
    Condition = {
