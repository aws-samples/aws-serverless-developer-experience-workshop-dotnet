    ...
    public async Task FunctionHandler(SQSEvent sqsEvent, ILambdaContext context)
    {
        foreach (var record in sqsEvent.Records)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"Requesting approval for property: {propertyId}");

                // Validate property ID
                if (string.IsNullOrWhiteSpace(propertyId) || !Regex.Match(propertyId, Pattern).Success)
                {
                    Console.WriteLine($"Input invalid; must conform to regular expression: {Pattern}");
                    return;
                }
                ...
                Console.WriteLine($"PK: {ddbKeys["pk"]}, SK: {ddbKeys["sk"]} ");
                
                // Query table for property 
                var properties = await QueryTableAsync(ddbKeys["pk"], ddbKeys["sk"]).ConfigureAwait(false);
                if (!properties.Any())
                {
                    Console.WriteLine("No property found in database with the requested property id");
                    return;
                }
                ...
                {
                    Console.WriteLine($"Property is already {property.Status}; no action taken");
                    return;
                }
                ...
                Console.WriteLine($"Stored item in DynamoDB;");
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
